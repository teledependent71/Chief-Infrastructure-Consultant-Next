...
import postsPageInitialPaths75a45Resource from '../../resources/posts-page-initial-paths-75a45'
import postsPageInitialProps74f2bResource from '../../resources/posts-page-initial-props-74f2b'
  ...
  return (
    <>
      <div className="posts11-container">
        <Head>
          <title>Posts1 - Chief Infrastructure Consultant</title>
          <meta
            property="og:title"
            content="Posts1 - Chief Infrastructure Consultant"
          />
        </Head>
        <DataProvider
...
export async function getStaticPaths() {
  try {
    const response = await postsPageInitialPaths75a45Resource({})
    return {
...
export async function getStaticProps(context) {
  try {
    const response = await postsPageInitialProps74f2bResource({
      ...context?.params,
    })
    if (!response?.data?.[0]) {
      return {
